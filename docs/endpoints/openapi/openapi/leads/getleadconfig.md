...
Endpoint: GET /v1/leads/config
Version: 1.9.0
Security: ApiKeyAuth

## Response 200 fields (application/json):

  - `addressMode` (string, required)
    Represents the mode of an input field
    Enum: "INACTIVE", "OPTIONAL", "MANDATORY"

  - `dateOfBirthMode` (string, required)
    Represents the mode of an input field
    Enum: "INACTIVE", "OPTIONAL", "MANDATORY"

  - `genderMode` (string, required)
    Represents the mode of an input field
    Enum: "INACTIVE", "OPTIONAL", "MANDATORY"

  - `telephoneMode` (string, required)
    Represents the mode of an input field
    Enum: "INACTIVE", "OPTIONAL", "MANDATORY"
  ...
  - `additionalInformation.name` (string, required)
    The name of the additional information field
    Example: "Do you have health issues?"

  - `additionalInformation.type` (string, required)
  ...
  - `additionalInformation.allowMultiSelection` (boolean, required)
    If the type is LIST this defines if only one or more items can be selected as answer

  - `additionalInformation.selectionValues` (array)
    A list of options to pick from. This is only used if the field type is a list.
    Example: ["Spinning","Yoga","Pilates"]

  - `additionalInformation.mode` (string, required)
    Represents the mode of an input field
    Enum: "INACTIVE", "OPTIONAL", "MANDATORY"
